...
{
	StaticObjectInfo(const string& objectName, float x, float y)
		: m_objectName(objectName), m_x(x), m_y(y) {}
...
public:
	...
	void AddStaticObjectInfo(const string& objectName, float x, float y) { m_staticObjectInfoList.emplace(objectName, new StaticObjectInfo(objectName, x, y)); }
