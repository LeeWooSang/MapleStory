...
bool LoginUILayer::Initialize()
{
	...
	m_objectMap.emplace(UI_OBJECT_KEY::LOGIN_BUTTON, loginButton);
	...
	m_objectMap.emplace(UI_OBJECT_KEY::ID_INPUT, idInput);
	...
	m_objectMap.emplace(UI_OBJECT_KEY::PW_INPUT, pwInput);
	...
	m_objectMap.emplace(UI_OBJECT_KEY::QUIT_INPUT, quitButton);
...
{	
	switch (flag)
	{
	case PROCESS_UI_COLLISION_TYPE::LOGIN_BUTTON_COLLISION_AND_CLICK:
	{
		string id = reinterpret_cast<TextUI*>(m_objectMap[UI_OBJECT_KEY::ID_INPUT])->ConvertTextToString();
		string pw = reinterpret_cast<TextUI*>(m_objectMap[UI_OBJECT_KEY::PW_INPUT])->ConvertTextToString();
		GET_INSTANCE(Network)->SendServerLoginPacket(id.c_str(), pw.c_str());
		cout << "로그인 버튼 클릭" << endl;
	}
	break;

	case PROCESS_UI_COLLISION_TYPE::ID_INPUT_COLLISION_AND_CLICK:
		m_inputKey = UI_OBJECT_KEY::ID_INPUT;
		reinterpret_cast<TextUI*>(object)->SetIsActive(true);
		reinterpret_cast<TextUI*>(m_objectMap[UI_OBJECT_KEY::PW_INPUT])->SetIsActive(false);
		break;

	case PROCESS_UI_COLLISION_TYPE::PW_INPUT_COLLISION_AND_CLICK:
		m_inputKey = UI_OBJECT_KEY::PW_INPUT;
		reinterpret_cast<TextUI*>(object)->SetIsActive(true);
		reinterpret_cast<TextUI*>(m_objectMap[UI_OBJECT_KEY::ID_INPUT])->SetIsActive(false);
		break;

	case PROCESS_UI_COLLISION_TYPE::QUIT_INPUT_COLLISION_AND_CLICK:
		::PostQuitMessage(0);
		break;
